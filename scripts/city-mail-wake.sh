  ...
  if [ -r "$CITY_MAIL_SIGNAL_FILE" ]; then
    ...
    if [ -n "$current_signal" ] && [ "$current_signal" != "$last_signal" ]; then
      if timeout --signal=TERM "$timeout_seconds" "$gc_command" --city "$CITY_PATH" \
        ...
        'You have authenticated tracker mail in Agent Mail. Fetch it through the Mayor-only Agent Mail MCP binding; follow Superpowers and IDD planning; respond on the linked Gitea issue before internal ceremony.' >/dev/null 2>&1; then
